...
class PerformanceMonitor {
  ...
  private maxMetricsToStore: number;
   ...
   * @param metricType - Type of metric (queryTime, apiCallTime, etc.)
   * @param value - Value to record
   */
  recordMetric(metricType: string, value: number): void {
    if (!this.metrics[`${metricType}s` as keyof PerformanceMetrics]) {
      // Create the array if it doesn't exist
      (this.metrics as any)[`${metricType}s`] = [];
    }

    // Add the value to the appropriate array
    const metricArray = this.metrics[`${metricType}s` as keyof PerformanceMetrics] as MetricItem[];
    metricArray.push({
      value: value,
    ...
    if (metricArray.length > this.maxMetricsToStore) {
      (this.metrics as any)[`${metricType}s`] = metricArray.slice(-this.maxMetricsToStore);
    }
  }

  /**
   * Check if a query time is within the performance threshold (90% under 2 seconds)
   * @param queryTime - Query time in milliseconds
   * @returns True if the query time is acceptable
   */
  isQueryTimeAcceptable(queryTime: number): boolean {
